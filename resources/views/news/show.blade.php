            ...
            <div class="d-flex justify-content-between">
                ...
                <div class="py-3 px-4 w-100 d-flex btn-share-group">
                    @php
                        ...
                        if( !empty($news->user_dislike) ){
                            $array = json_decode($news->user_dislike, true);
                    ...
                    @endphp
                    <button id="btn_like_item" class="btn btn-like {{ $check_like }} me-1"  onclick="action_btnlike_dislike(this.className);return create_logs('Like ข่าวสาร/กิจกรรม_{{ $news->title }}');">
                    ...
                    <button id="btn_dislike_item" class="btn btn-dislike {{ $check_dislike }} me-1"   onclick="action_btnlike_dislike(this.className);return create_logs('Dislike ข่าวสาร/กิจกรรม_{{ $news->title }}');">
                        <div class="icon-btn">
                            <i class="fa-solid fa-thumbs-down"></i>
                        </div>
                    </button>
                    <button class="btn btn-share me-1"  onclick="click_share_news();return create_logs('Share ข่าวสาร/กิจกรรม_{{ $news->title }}');">
                ...
                @php
                    ...
                    if( !empty($news->user_fav) ){
                        $array = json_decode($news->user_fav, true);
                        $user_id = Auth::user()->id ;
                ...
                <div class="{{ $check_fav }} px-2 py-0 d-flex align-items-start btn-bookmark-training cursor-pointer" onclick="action_btnlike_dislike(this.className);return create_logs('Fav ข่าวสาร/กิจกรรม_{{ $news->title }}');">
            ...
            <div class="px-4 title-training">
                ...
                <div id="div_show_rating" class="rating-training mt-2">
                    <span id="sum_rating_span" style="color: #EDB529;font-size: 14px;font-style: normal;font-weight: 600;line-height: normal;margin-right: 5px;">
                        {{ number_format($news->sum_rating, 1) }}
                    </span>
                    <i id="sum_rating_i" data-star="{{ number_format($news->sum_rating, 1) }}" class="star-rating"></i>
                </div>
                @else
            ...
            <div class="detail-training">
                ...
                <div class="w-100 mt-4">
                    ...
                    <div class="d-flex justify-content-end ">
                        <button class="btn btn-like {{ $check_like }}  me-1"  onclick="action_btnlike_dislike(this.className);return create_logs('Like ข่าวสาร/กิจกรรม_{{ $news->title }}');">
                            <div class="icon-btn d-flex">
                                <i class="fa-solid fa-thumbs-up"></i>
                            </div>
                        ...
                        <button class="btn btn-dislike {{ $check_dislike }} me-1"  onclick="action_btnlike_dislike(this.className);return create_logs('Dislike ข่าวสาร/กิจกรรม_{{ $news->title }}');">
                            <div class="icon-btn">
                                <i class="fa-solid fa-thumbs-down"></i>
                            </div>
                        </button>
...
<div class="modal fade" id="rating" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content modal-vote-training">
            <p class="text-white text-center mb-5 font-20">ให้คะแนนข่าวนี้</p>
            ...
            <div class="w-100 px-5 mt-3">
                <button class="btn w-100 bg-white btn-vote-training" data-dismiss="modal" aria-label="Close" onclick="getRating('{{ $news->id }}')">ให้คะแนน</button>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="dislike_training" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content modal-vote-training">
            <p class="text-white text-center mb-5 font-20">ให้เหตุผลที่ไม่ชอบข่าวนี้</p>
            <div class="w-100 d-flex justify-content-center px-4 mb-2">
                <textarea class="form-control" id="reasons_dislike" placeholder="กรอกเหตุผลที่ไม่ชอบข่าวนี้" rows="5" style="border-radius: 10px;" oninput="check_reasons_dislike()"></textarea>
            </div>
...
<script>
    ...
    function update_user_view(){
        ...
        fetch("{{ url('/') }}/api/update_user_view_news/" + "{{ Auth::user()->id }}" + "/" + news_id)
    ...
    function action_btnlike_dislike(className) {
        ...
        if (className.includes('btn-like')) {
            ...
            if (!className.includes('active')) {
                // console.log('FAV');
                user_click_fav_btn('Yes' , news_id);
            }
            else{
                // console.log('ยกเลิก FAV');
                user_click_fav_btn('No' , news_id);
            }
        }
    }
    ...
    function getRating(news_id) {
        ...
        fetch("{{ url('/') }}/api/give_rating_news/" + "{{ Auth::user()->id }}" + "/" + news_id + "/" + selectedRating)
            .then(response => response.text())
            .then(result => {
                // console.log(result);
    ...
    function user_cancel_like(){
        ...
        let news_id = "{{ $news->id }}";
        // console.log(news_id);
        fetch("{{ url('/') }}/api/user_cancel_like_news/" + "{{ Auth::user()->id }}" + "/" + news_id)
            .then(response => response.text())
            .then(result => {
                // console.log(result);
                let formattedResult = parseFloat(result).toFixed(1);
    ...
    function user_cancel_dislike(){
        let news_id = "{{ $news->id }}";
        // console.log(news_id);
        fetch("{{ url('/') }}/api/user_cancel_dislike_news/" + "{{ Auth::user()->id }}" + "/" + news_id)
            .then(response => response.text())
            .then(result => {
                // console.log(result);
            });
    ...
    function submit_reasons_dislike(){
        ...
        let news_id = "{{ $news->id }}";
        // console.log(news_id);
        fetch("{{ url('/') }}/api/submit_reasons_dislike_news/" + "{{ Auth::user()->id }}" + "/" + news_id + "/" + reasons_dislike)
            .then(response => response.text())
            .then(result => {
                // console.log(result);
            });
    ...
    function user_click_fav_btn(type , news_id){

        fetch("{{ url('/') }}/api/user_click_fav_btn_news/" + "{{ Auth::user()->id }}" + "/" + news_id + "/" + type)
            .then(response => response.text())
            .then(result => {
                // console.log(result);
            });
...
<script>
    if(document.getElementById('trainingVideo')){
        ...
        window.addEventListener('beforeunload', function(e) {
            ...
            if(countTime > 0){
                fetch("{{ url('/') }}/api/update_countTime_newsVideo/" + "{{ Auth::user()->id }}" + "/" + countTime + "/" + news_id)
                    .then(response => response.text())
                    .then(result => {
                        // console.log(result);
                    });
    ...
    function click_share_news(){
        ...
        let url = "{{ url('/') }}/" + "share_news/" + news_id ;
        ...
        tag_a_share_line.setAttribute('onclick' , "save_log_share('news' , 'line' , '"+news_id+"')");
        tag_a_share_facebook.setAttribute('onclick' , "save_log_share('news' , 'facebook' , '"+news_id+"')");
        tag_a_share_twitter.setAttribute('onclick' , "save_log_share('news' , 'twitte' , '"+news_id+"')");
        tag_a_share_whatsapp.setAttribute('onclick' , "save_log_share('news' , 'whatsapp' , '"+news_id+"')");
        input_for_copy.setAttribute('onclick' , "save_log_share('news' , 'copy' , '"+news_id+"')");
        btn_for_copy_Share.setAttribute('onclick' , "copy_Share_social_media();save_log_share('news' , 'copy' , '"+news_id+"')");
