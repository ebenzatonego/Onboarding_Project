...
<div class="row">
    ...
    <div class="col-md-9 pt-3">
        ...
        <dt class="text-info">#{{ $news->name_type }}</dt>

        <div >
            <p class="card-text fs-6 mt-3" style="flex-wrap: wrap;display: flex;text-wrap: wrap;width: 100px !important;">
                {{ $news->detail }}
            </p>
        </div>
        ...
        <div class="mt-3">
            ...
            <span id="create_by">{{ $news->name_creator }}</span>
        </div>

        @php
			// แปลงสตริง JSON เป็นอาร์เรย์ PHP
			$user_like_array = json_decode($news->user_like, true);
        ...
        <h4 class="mt-4">
            ...
            <span>{{ number_format($news->sum_rating) }} ดาว</span>
        </h4>
    </div>
</div>
...
<script>
	...
	document.addEventListener('DOMContentLoaded', async function () {
  	...
  	function get_log_view(){
        return new Promise((resolve) => {
            ...
            if("{{ $news->user_view }}"){
        	    ...
        	    let db_log_view = @json($news->user_view);
  	...
  	function get_log_like(){
        return new Promise((resolve) => {
            ...
            if("{{ $news->user_like }}"){
          		// ดึงข้อมูล JSON จาก Blade Template
        	    let db_log_like = @json($news->user_like);
    ...
    function get_log_rating(){
        return new Promise((resolve) => {
            ...
            if("{{ $news->log_rating }}"){
                ...
                // ดึงข้อมูล JSON จาก Blade Template
                let db_log_rating = @json($news->log_rating);
    ...
    function get_logs_dislike(){
        return new Promise((resolve) => {
            ...
            if("{{ $news->user_dislike }}"){

                // ดึงข้อมูล JSON จาก Blade Template
                let db_log_dislike = @json($news->user_dislike);
    ...
    function get_logs_fav(){
        return new Promise((resolve) => {
            ...
            if("{{ $news->user_fav }}"){
                ...
                // ดึงข้อมูล JSON จาก Blade Template
                let db_log_fav = @json($news->user_fav);
    ...
    function get_logs_share(){
        return new Promise((resolve) => {
            ...
            if("{{ $news->user_share }}"){
                ...
                // ดึงข้อมูล JSON จาก Blade Template
                let db_log_share = @json($news->user_share);
    ...
    function get_logs_video(){
        return new Promise((resolve) => {
            ...
            if("{{ $news->log_video }}"){
                ...
                // ดึงข้อมูล JSON จาก Blade Template
                let db_log_video = @json($news->log_video);
