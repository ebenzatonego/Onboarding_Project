...
<div class="row">
	<div class="col-12">
		<div class="card">
			<div class="row g-0">
				<div class="col-md-6">
					@if( !empty($content_popups->video) )
						<video id="tag_video_content_popups" src="{{ $content_popups->video }}" controls muted style="width:100%;border-radius: 10px; max-width: 700px;" class="video-preview"></video>
					@elseif(!empty($content_popups->photo))
						<img id="tag_photo_content_popups" src="{{ $content_popups->photo }}" style="width:100%;border-radius: 10px; max-width: 700px;" class="video-preview">
					@endif
				...
				<div class="col-md-6">
					<div class="card-body p-3">
						...
						@if( !empty($content_popups->detail) )
						<p class="card-text">
							รายละเอียด
						</p>
						<div class="card">
							<div class="card-body">
								{!! $content_popups->detail !!}
							</div>
						</div>
						@endif
					</div>
...
@if( !empty($content_popups->log_video) )
...
function change_format_date(){
	let datetime = "{{ $content_popups->created_at }}";
	// แยกวันที่และเวลา
...
function change_format_log(){
	...
	if("{{ $content_popups->log_video }}"){
		// แปลง HTML encoded string ให้เป็น string ปกติ
		let log_DB = "{{ $content_popups->log_video }}";
