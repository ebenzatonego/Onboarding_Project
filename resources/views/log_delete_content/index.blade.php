    ...
    <div class="card-body p-5">
        <div>
            ...
            <div id="div_for_export" class="row mt-4 d-none">
                ...
                <div class="col-2">
                    <div class="input-group">
                        <span class="input-group-text bg-transparent"><i class="fa-solid fa-magnifying-glass"></i></span>
                        <input type="text" class="form-control border-start-0" id="search_log_content" placeholder="Log Delete" oninput="delay_search_data_in_card();">
                    </div>
                </div>
                <div class="col-2">
        ...
        <div class="col-12 mt-2">
        	<div class="row mb-2">
                ...
                <div class="col-2"><h6>Log Delete</h6></div>
                <div class="col-2"><h6>type</h6></div>
...
<script>
    ...
    async function fetch_log_web(page) {
        ...
        try {
            const response = await fetch(`{{ url('/') }}/api/get_list_log_web_delete?page=${page}&limit=${limit}`);
            ...
            result.forEach(item => {
                ...
                let log_content_name;
                if (item.news_name) {
                    log_content_name = item.news_name;
                } 
                if(item.training_name) {
                    log_content_name = item.training_name;
                }
                if(item.product_name) {
                    log_content_name = item.product_name;
                }
                if(item.appointment_name) {
                    log_content_name = item.appointment_name;
                }
                if(item.activity_name) {
                    log_content_name = item.activity_name;
                }
				let html = `
				    <div name_user="${item.name}" account="${item.account}" role="${item.role}" log_content="${log_content_name}" current_rank="${item.current_rank}" class="log-item data_log_web">
				        <div class="card w-100 shadow-sm border-1 border p-3 mt-1">
				            <div class="row view_item_of_user">
				                ...
				                <div class="col-2 excel_log_content">${log_content_name}</div>
				                <div class="col-2 excel_log_type">${item.type}</div>
	...
	function exportExcel() {
        ...
        sheetNames.forEach(sheetName => {
            ...
            logItems.forEach(logItem => {
                ...
                viewItems.forEach(viewItem => {
                    ...
                    const excel_log_type = viewItem.querySelector('.excel_log_type').textContent;
        ...
        const filename = `log_delete_${datetime}.xlsx`;
