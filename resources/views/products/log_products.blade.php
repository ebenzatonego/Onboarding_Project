...
<div class="row">
    ...
    <div class="col-md-9 pt-3">
        ...
        <dt class="text-info">#{{ $product->name_type }}</dt>
        ...
        <div class="mt-3">
            ...
            <span id="create_by">{{ $product->name_creator }}</span>
        </div>

        @php
			// แปลงสตริง JSON เป็นอาร์เรย์ PHP
			$user_like_array = json_decode($product->user_like, true);
        ...
        <h4 class="mt-4">
            ...
            <span>{{ number_format($product->sum_rating) }} ดาว</span>
        </h4>
    </div>
</div>
...
<script>
	...
	document.addEventListener('DOMContentLoaded', async function () {
  	...
  	function get_log_view(){
        return new Promise((resolve) => {
            ...
            if("{{ $product->user_view }}"){

        	    let db_log_view = @json($product->user_view);
  	...
  	function get_log_like(){
        return new Promise((resolve) => {
            ...
            if("{{ $product->user_like }}"){
          		// ดึงข้อมูล JSON จาก Blade Template
        	    let db_log_like = @json($product->user_like);
    ...
    function get_log_rating(){
        return new Promise((resolve) => {
            ...
            if("{{ $product->log_rating }}"){

                // ดึงข้อมูล JSON จาก Blade Template
                let db_log_rating = @json($product->log_rating);
    ...
    function get_logs_dislike(){
        return new Promise((resolve) => {
            ...
            if("{{ $product->user_dislike }}"){

                // ดึงข้อมูล JSON จาก Blade Template
                let db_log_dislike = @json($product->user_dislike);
    ...
    function get_logs_fav(){
        return new Promise((resolve) => {
            ...
            if("{{ $product->user_fav }}"){
                ...
                // ดึงข้อมูล JSON จาก Blade Template
                let db_log_fav = @json($product->user_fav);
    ...
    function get_logs_share(){
        return new Promise((resolve) => {
            ...
            if("{{ $product->user_share }}"){

                // ดึงข้อมูล JSON จาก Blade Template
                let db_log_share = @json($product->user_share);
