...
<div class="row">
	<div class="col-12">
		<div class="card">
			<div class="row g-0">
				<div class="col-md-6">
					<video id="tag_video_intro" src="{{ $video_congrats->video }}" controls muted style="width:100%;border-radius: 10px; max-width: 700px;" class="video-preview"></video>
				</div>
				<div class="col-md-6">
					<div class="card-body p-3">
						<h4 class="card-title">
							<b>{{ $video_congrats->name_video }}</b>
						</h4>
						<p class="card-text">
							<span class="text-dark" style="width:30%;">
								ระดับที่แสดงผล : <b>{{ $video_congrats->for_rank }}</b>
							</span>
						</p>
						<p class="card-text">
...
function change_format_date(){
	let datetime = "{{ $video_congrats->created_at }}";
	// แยกวันที่และเวลา
...
function change_format_log(){
	...
	if("{{ $video_congrats->log }}"){
		// แปลง HTML encoded string ให้เป็น string ปกติ
		let log_DB = "{{ $video_congrats->log }}";
