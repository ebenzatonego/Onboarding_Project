...
<style>
  ...
  .long-item {
    ...
    .long-item .count-news {
        position: absolute;
        bottom: 15px;

    }

    .square-item .count-news {
        position: absolute;
        bottom: 10px;
...
<button id="btn_Modal_delete_news_type" type="button" class="btn btn-primary d-none" data-toggle="modal" data-target="#Modal_delete_news_type">
  Launch demo modal
</button>

<!-- Modal -->
<div class="modal fade" id="Modal_delete_news_type" tabindex="-1" aria-labelledby="Label_delete_news_type" aria-hidden="true">
      ...
      <div class="modal-body">
        <div class="mt-3 mb-2 text-center">
          ...
          <h4 class="mt-3">
            ยืนยันการลบ
            <br>
            <b><span id="modal_delete_name_news_type"></span></b>
          </h4>
          <p class="text-danger"><u>เมื่อกดยืนยัน ข่าวทั้งหมดที่เป็นประเภทนี้จะถูกลบด้วย</u></p>
        </div>
        <hr>
        <center>
          <button id="btn_close_Modal_delete_news_type" type="button" class="btn btn-secondary" data-dismiss="modal" style="width:40%;">
            ปิด
          </button>
          <button id="btn_cf_delete_news_type" type="button" class="btn btn-danger" style="width:40%;">
            ยืนยันการลบ
          </button>
        </center>
      </div>
...
<div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
	<div class="breadcrumb-title pe-3">การจัดการข่าว</div>
  <div class="ps-3">
    <select id="select_show_news_by_type" class="form-select" onchange="change_show_news_by_type();">
	...
	<div class="ms-auto">
		<div class="btn-group">
			<a href="{{ url('/news/create') }}" class="btn btn-success">
				<i class="fa-solid fa-calendar-plus"></i> เพิ่มข่าว
			</a>
		</div>
		<div class="btn-group">
...
<div class="card border-top border-0 border-4 border-dark">
  <div class="card-body p-5">
    <div class="card-title d-flex align-items-center">
      <h5 id="h5_news_types" class="mb-0 text-dark">
        <!--  -->
      </h5>
    </div>
    <hr>
...
<script>
	...
	document.addEventListener('DOMContentLoaded', function () {
        get_data_news('all');
        get_data_News_type();
        get_data_number_menu_of_news();
  });

  function get_data_news(type){
    fetch("{{ url('/') }}/api/get_data_news_admin/" + type )
      .then(response => response.json())
      .then(result => {
          // console.log(result);

          if(type == 'all'){
              document.querySelector('#h5_news_types').innerHTML = 'ข่าวทั้งหมด';
          }
          else{
              document.querySelector('#h5_news_types').innerHTML = result['name_type'];
          }
          ...
          if(result['data_news']){
              for (let i = 0; i < result['data_news'].length; i++) {

                  let textWithoutHtml = `<br><br><br>`;
                  if(result['data_news'][i].detail){
                      textWithoutHtml = result['data_news'][i].detail.replace(/(<([^>]+)>)/gi, "");
                  }

                  // name_type
                  let name_type = `` ;
                  if(type == 'all'){
                      name_type = result['data_news'][i].name_type;
                  }
                  else{
                  ...
                  let html_Highlight ;
                  if(type == 'all'){
                    html_Highlight = `<span id="span_Highlight_id_`+result['data_news'][i].id+`" class="float-end">..</span>`;
                    if(result['data_news'][i].highlight_number){
                      html_Highlight = `
                        <span id="span_Highlight_id_`+result['data_news'][i].id+`">
                          <i class="i_Highlight fa-solid fa-circle-`+result['data_news'][i].highlight_number+` font-24 float-end text-success"></i>
                        </span>
                        `;
                    }
                  }else{
                    html_Highlight = `<span id="span_Highlight_id_`+result['data_news'][i].id+`" class="float-end">..</span>`;
                    if(result['data_news'][i].highlight_of_type){
                      html_Highlight = `
                        <span id="span_Highlight_id_`+result['data_news'][i].id+`">
                          <i class="i_Highlight fa-solid fa-circle-`+result['data_news'][i].highlight_of_type+` font-24 float-end text-success"></i>
                        </span>
                        `;
                    }
                  }
                  ...
                  if(result['data_news'][i].datetime_start){
                    let datetime_start = result['data_news'][i].datetime_start;
                    // แยกวันที่และเวลา
                  ...
                  if(result['data_news'][i].datetime_end){
                    let datetime_end = result['data_news'][i].datetime_end;
                    // แยกวันที่และเวลา
                  ...
                  `;
                  if(result['data_news'][i].status == 'Yes'){
                    html_status = `
                  ...
                  let html = `
                      <div class="col-12 col-md-3">
                        <div class="card" style="position:relative;">
                          `+html_status+`
                          <img src="`+result['data_news'][i].photo_cover+`" class="card-img-top">
                          <div class="card-body">
                            <h5 class="card-title">
                              `+result['data_news'][i].title+`
                            </h5>
                            ...
                            <center class="mt-4 mb-2">
                              ...
                              <div class="row">
                                <div class="col-12">
                                  <a href="{{ url('/preview_news') }}/`+result['data_news'][i].id+`" class="btn btn-sm btn-info w-100">
                                  ...
                                  @if(Auth::check())
                                    ...
                                    <div class="btn-group w-100" role="group">
                                      ...
                                      <ul class="dropdown-menu" style="margin: 0px;">
                                        <li>
                                          <span class="dropdown-item btn" onclick="change_Highlight('`+result['data_news'][i].id+`' , 'ว่าง' , '`+type+`')">
                                            ว่าง
                                          </span>
                                        </li>
                                        <li>
                                          <span class="dropdown-item btn" onclick="change_Highlight('`+result['data_news'][i].id+`' , '1' , '`+type+`')">
                                            ลำดับที่ 1
                                          </span>
                                        </li>
                                        <li>
                                          <span class="dropdown-item btn" onclick="change_Highlight('`+result['data_news'][i].id+`' , '2' , '`+type+`')">
                                            ลำดับที่ 2
                                          </span>
                                        </li>
                                        <li>
                                          <span class="dropdown-item btn" onclick="change_Highlight('`+result['data_news'][i].id+`' , '3' , '`+type+`')">
                                            ลำดับที่ 3
                                          </span>
                                        </li>
                                        <li>
                                          <span class="dropdown-item btn" onclick="change_Highlight('`+result['data_news'][i].id+`' , '4' , '`+type+`')">
                                            ลำดับที่ 4
                                          </span>
                                        </li>
                                        <li>
                                          <span class="dropdown-item btn" onclick="change_Highlight('`+result['data_news'][i].id+`' , '5' , '`+type+`')">
                                            ลำดับที่ 5
                                          </span>
                                        </li>
                                      </ul>
  ...
  function change_Highlight(news_id , number , type){
    // console.log(news_id);
    // console.log(number);

    fetch("{{ url('/') }}/api/change_Highlight_news/" + news_id  + "/" + number + "/" + type)
      .then(response => response.json())
      .then(result => {
          // console.log(result);
          ...
          let select = document.querySelector('#span_Highlight_id_'+news_id);
          select.innerHTML = `
            <span id="span_Highlight_id_`+news_id+`">
              <i class="i_Highlight fa-solid fa-circle-`+number+` font-24 float-end text-success"></i>
            </span>
          `;

          let icon_2 = document.getElementById('span_Highlight_id_'+news_id);
  ...
  function get_data_News_type(){
    ...
    let select_show_news_by_type = document.querySelector('#select_show_news_by_type');

    fetch("{{ url('/') }}/api/get_data_News_type")
      .then(response => response.json())
      .then(result => {
        //   console.log(result);
          ...
          if(result){
            for(let item of result){
                ...
                select_show_news_by_type.add(option);             
            } 
          }
    });
  ...
  function change_show_news_by_type(){
    let select_news_id = document.querySelector('#select_show_news_by_type').value ;
    get_data_news(select_news_id)
  }
  ...
  function get_data_number_menu_of_news(){
    ...
    let content_number_menu_type = document.querySelector('#content_number_menu_type');
        ...
        fetch("{{ url('/') }}/api/get_data_number_menu_of_news")
          .then(response => response.json())
          .then(result => {
              // console.log(result);
                ...
                for (let i = 0; i < result.length; i++) {
                  ...
                  let html = `
                    ...
                    <div class="col-3 center-vertical mb-3">
                      ...
                      <button type="button" class="btn btn-sm btn-danger mx-2" onclick="click_delete_name_news_type('`+result[i].id+`' , '`+result[i].name_type+`')">
                        <i class="fa-solid fa-trash-can"></i> ลบ
                      </button>
                    </div>
                    <hr>
  ...
  function change_number_menu_type(type_id , number){
    ...
    fetch("{{ url('/') }}/api/change_number_menu_type_news/"+ type_id + "/" + number)
  ...
  function click_delete_name_news_type(news_type_id , name_type){

    let btn = document.querySelector('#btn_cf_delete_news_type');
    let name_news_type = document.querySelector('#modal_delete_name_news_type');
        name_news_type.innerHTML = name_type ;

        btn.setAttribute('onclick' , "cf_delete_news_type('"+news_type_id+"')");
        
        document.querySelector('#btn_close_modal_menu_management').click();
        document.querySelector('#btn_Modal_delete_news_type').click();
  }

  function cf_delete_news_type(news_type_id){
    fetch("{{ url('/') }}/api/delete_news_type/"+ news_type_id)
          .then(response => response.text())
          .then(result => {
              // console.log(result);

            if(result == 'success'){
              document.querySelector('#btn_close_Modal_delete_news_type').click();
              get_data_number_menu_of_news();
            }
    });
  }
